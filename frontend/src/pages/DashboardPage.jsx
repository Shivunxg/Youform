...
import { MoreHorizontal, Edit2, BarChart2, Copy, Trash2, ExternalLink, Users } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg px-3 py-1.5 hover:bg-gray-50">
              <div className="w-4 h-4 rounded bg-gray-200" />
              My Workspace
              <span className="text-gray-400">▾</span>
            </button>
            <button onClick={() => navigate('/settings/members')} className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 transition-colors">
...
function FormCard({ form, menuOpen, onMenuToggle, onEdit, onResponses, onAnalytics, onDuplicate, onDelete }) {
  return (
    <div
      className="bg-white rounded-2xl border border-gray-200 hover:shadow-md transition-all duration-150 group cursor-pointer overflow-hidden"
      onClick={onEdit}
    >
      {/* Colored top */}
...
function MenuItem({ icon: Icon, label, onClick, danger }) {
  return (
    <button
      onClick={onClick}
      className={clsx('w-full flex items-center gap-2 px-3 py-2 text-sm transition-colors',
  ...
  return (
    <div className="text-center py-24">
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <button onClick={() => navigate('/settings/members')} className="flex items-center gap-2 px-5 py-2.5 border border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50">
