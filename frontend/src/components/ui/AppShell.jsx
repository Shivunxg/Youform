...
import { Bell, ChevronDown, Settings, LogOut, User, LayoutDashboard, FileText, Import, Menu, X } from 'lucide-react';
...
export default function AppShell({ children }) {
  ...
  const navLinkClass = ({ isActive }) => clsx(
    'flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all border-2',
    isActive
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF2]">
      ...
      <header className="bg-white border-b-2 border-[#111] h-14 flex items-center px-4 gap-3 sticky top-0 z-30">
        ...
        {/* Logo */}
        <button onClick={() => { navigate('/dashboard'); closeAll(); }} className="flex items-center gap-2 shrink-0">
        ...
        <div className="relative hidden md:block">
          <button
            onClick={() => { setShowWsMenu(v => !v); setShowUserMenu(false); }}
          ...
          >
            ...
            <span className="max-w-[120px] truncate">{activeWs?.name ?? 'Select workspace'}</span>
            <ChevronDown className="w-3.5 h-3.5 text-[#111] shrink-0" />
          </button>
              ...
              {workspaces.map(ws => (
                <button key={ws.id}
                  ...
                  <div className="w-6 h-6 rounded-md border-2 border-[#111] flex items-center justify-center text-xs font-bold text-white"
                    style={{ background: '#111' }}>{ws.name?.[0]}</div>
                  {ws.name}
                </button>
              ))}
        ...
        <nav className="hidden md:flex items-center gap-1 ml-2">
          <NavLink to="/dashboard"      className={navLinkClass} style={{ fontWeight: 700, ...SG }}>
            <LayoutDashboard className="w-4 h-4" /> Dashboard
          </NavLink>
          <NavLink to="/templates"      className={navLinkClass} style={{ fontWeight: 700, ...SG }}>
            <FileText className="w-4 h-4" /> Templates
          </NavLink>
          <NavLink to="/import/typeform" className={navLinkClass} style={{ fontWeight: 700, ...SG }}>
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <div className="relative">
            <button
              ...
              <span className="text-sm text-[#111] hidden lg:block" style={{ fontWeight: 700, ...SG }}>
              ...
              <div className="w-8 h-8 rounded-full border-2 border-[#111] flex items-center justify-center text-white text-sm font-bold"
                style={{ background: '#111' }}>{initials}</div>
            </button>
            {showUserMenu && (
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-lg border-2 border-[#111] text-[#111] transition-colors"
      ...
      {/* ── Mobile drawer ───────────────────────────────────────────────── */}
      {mobileOpen && (
        <div className="md:hidden bg-white border-b-2 border-[#111] px-4 py-4 flex flex-col gap-2 sticky top-14 z-20"
          style={{ boxShadow: '0 4px 0 #111' }}>

          {/* Workspace */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border-2 border-[#111] text-sm font-bold text-[#111] bg-[#FFFBF2]"
            style={SG}>
            <div className="w-5 h-5 rounded-md border-2 border-[#111] flex items-center justify-center shrink-0"
              style={{ background: '#f97316' }}>
              <span className="text-white text-xs font-bold">{activeWs?.name?.[0] ?? 'W'}</span>
            </div>
          ...
          <NavLink to="/dashboard" onClick={closeAll}
            className={({ isActive }) => clsx('flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold border-2 transition-all', isActive ? 'bg-[#111] text-white border-[#111]' : 'text-[#111] border-transparent hover:border-[#111]')}
            style={SG}>
            <LayoutDashboard className="w-4 h-4" /> Dashboard
          </NavLink>
          <NavLink to="/templates" onClick={closeAll}
            className={({ isActive }) => clsx('flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold border-2 transition-all', isActive ? 'bg-[#111] text-white border-[#111]' : 'text-[#111] border-transparent hover:border-[#111]')}
            style={SG}>
            <FileText className="w-4 h-4" /> Templates
          </NavLink>
          <NavLink to="/import/typeform" onClick={closeAll}
            className={({ isActive }) => clsx('flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold border-2 transition-all', isActive ? 'bg-[#111] text-white border-[#111]' : 'text-[#111] border-transparent hover:border-[#111]')}
            style={SG}>
            <Import className="w-4 h-4" /> Import
          </NavLink>
          ...
          {!isPro && (
            <button onClick={() => { navigate('/settings/billing'); closeAll(); }}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-white text-sm rounded-lg border-2 border-[#111]"
              style={{ background: '#f97316', fontWeight: 700, ...SG, boxShadow: '3px 3px 0 #111' }}>
              ⚡ Upgrade to Pro
            </button>
          )}
          <button onClick={() => { navigate('/settings/account'); closeAll(); }}
            className="w-full flex items-center gap-2 px-3 py-2.5 text-sm font-bold text-[#111] rounded-lg border-2 border-transparent hover:border-[#111] transition-colors"
            style={SG}>
            <User className="w-4 h-4" /> Account Settings
          </button>
